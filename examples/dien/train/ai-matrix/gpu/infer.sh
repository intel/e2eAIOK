...
do
	...
	# numactl -N 0 python script/train.py --mode=test --batch_size=$batch |& tee results/result_infer_${batch}.txt
        numactl -C 0-0,40-40 --membind=0 python script/train.py --mode=test --advanced --batch_size=$batch --num-inter-threads=20 --num-intra-threads=20 |& tee results/result_infer_${batch}.txt
	end=`date +%s%N`
