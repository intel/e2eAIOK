...
def calculate(sess, test_data, input_tensor, output_tensor, batch_size):
    # nums = 0
    # total_data = []
    ...
    # prepare_end = time.time()
    # print("prepare time   ", prepare_end - prepare_start)
    # # raise Exception("prepare data")

    # # prepared_filename = "dien_data_infer_prepared_batch_{}.pickle".format(batch_size)
    # # with open(prepared_filename, 'wb') as f:
    # #     pickle.dump(total_data, f)
    
    
    # When first run this repo, please uncomment the code above (line 111-158) to generate pickle file
    # the pickle data is used to avoid process the raw data repeatedily
    prepared_filename = "dien_data_infer_prepared_batch_{}.pickle".format(batch_size)
    ...
    for i in range(1, total_samples + 1):
        ...
        if i == 70000:
            ...
            with open('./timeline/dien_1core_1inst_infer_timeline_intel-disable2-1.15_8260_step{}_inter1intra1omp1_0819.json'.format(i), 'w') as f:
    ...
    # with open('./times/dien_1core_1inst_infer_timeline_im0617allmklinput_tanhfusion_8260_step{}_inter1intra1omp1_0700.txt', 'w') as wf:
...
def inference(pb_path,      
    ...
    # session_config.intra_op_parallelism_threads = 20
