...
def test(
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(
        ...
        if model_type == 'DNN':
            ...
            model = Model_DIN_V2_Gru_Vec_attGru_Neg(n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, data_type, device='cpu')
        else:
            print ("Invalid model_type : %s", model_type)
            return
        # for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
        ...
        if data_type == 'FP32':
            ...
            frozen_graph_def = tf.graph_util.convert_variables_to_constants(
                ...
                sess.graph_def,
                output_node_names)
            tf.io.write_graph(frozen_graph_def, '/home2/yunfeima/tmp/dien', 'constant_sub_node_graph_cpu.pb',as_text=False)
            # exit(0)
