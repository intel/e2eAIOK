...
def load_raw(spark, folder, day_range,raw_data_file):
    label_fields = [StructField('_c%d' % LABEL_COL, IntegerType())]
    int_fields = [StructField('_c%d' % i, IntegerType()) for i in INT_COLS]
    str_fields = [StructField('_c%d' % i, StringType()) for i in CAT_COLS]

    schema = StructType(label_fields + int_fields + str_fields)
    return spark.read.schema(schema).option('sep', '\t').csv(raw_data_file)
...
##### Descrption 
# Generate_modes:
# 1. load raw data with spark.read.schema, seperate w/ \t
# 2. get_column_counts counts
# 3. write columns to combined.parquet 
# 4. read combined column data, iterate it, and write to seperate parquet indexed by col 

def _main():
    args = _parse_args()
    spark = SparkSession.builder.getOrCreate()
    
    df = load_raw(spark, args.input_folder, args.day_range,args.raw_data_file)
...
#####
# 1. create a random long from the double precision flot
# 2. apply_model: sort the models so broadcast joins come first, this is to reduce shuffle data 
# 3. fill na with transformat_log 
