...
def _parse_args():
    ...
    parser.add_argument('--train_days', required=True)
    parser.add_argument('--remain_days', required=True)
    parser.add_argument('--input_folder', required=True)
    parser.add_argument('--test_input_folder', required=True)
    parser.add_argument('--validation_input_folder', required=True)
    ...
    start, end = args.remain_days.split('-')
    args.remain_day_range = list(range(int(start), int(end) + 1))
    args.remain_days = len(args.remain_day_range)

    start, end = args.train_days.split('-')
    args.train_day_range = list(range(int(start), int(end) + 1))
    args.train_days = len(args.train_day_range)
...
def _main():
    ...
    # Using Spark UDF to convert parquet to binary files 
    def func(iterator):
        dst_file = convert_output_folder+str(uuid.uuid1())[0:8]+".bin"
        with open(dst_file,'ab') as f: 
            for pdf in iterator:
                temp_df=pdf    
                pdf = pdf[all_columns_sorted]
                pdf[dense_columns] = pdf[dense_columns].astype(np.float32)
    ...
    if args.mode == 'generate_models':
        ...
        time_end = time()
        print(F"Generate models cosuming time:{time_end-time_start}")
    ...
    if args.mode == 'transform':
        ...
        with _timed('transform'):
            ...
            #Save spark dataframe to binary files on local disk        
    ...
    args.days =args.remain_days
    df.unpersist(True)
    args.input_folder = args.test_input_folder
    df = load_raw(spark, args.input_folder, args.day_range)
    ...
    df = load_raw(spark, args.input_folder, args.day_range)
    convert_output_folder = "/mnt/DP_disk5/validation/"
    if args.mode == 'transform':
        time_start = time()
        spark.conf.set('spark.sql.shuffle.partitions', args.days * args.apply_shuffle_parallel_per_day*8)
        with _timed('transform'):
            ...
            df.mapInPandas(func, schema=df.schema).describe()
        time_end = time()
        print(F"Transform validation data cosuming time:{time_end-time_start}")
    
    time2 = time()
    print('=' * 100)
    print(_benchmark)
    print(F"Total Consuming time:{time2-time1}")
