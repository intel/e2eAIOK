...
# Default train dataset size; train set rows number, get from preproc3#397
...
def main(FLAGS):
    ...
    local_eval_batch_size = FLAGS.eval_batch_size // num_gpus
    create_batches = local_batch_size // FLAGS.prebatch_size
    eval_create_batches = local_eval_batch_size // FLAGS.prebatch_size
    ...
    if FLAGS.predict or FLAGS.evaluate:  # inference
        ...
        if FLAGS.benchmark:
            ...
            early_stop = tf.estimator.experimental.stop_if_higher_hook(estimator, 'map_with_leak', threshold=0.67632, run_every_secs=None, run_every_steps=100)
            hooks.append(early_stop)
            # profile_hook = tf.estimator.ProfilerHook(save_steps=10, output_dir="out")
            # hooks.append(profile_hook)
            #summary_hook = tf.estimator.SummarySaverHook(save_steps=100, output_dir="summary", scaffold=tf.train.Scaffold(summary_op=tf.summary.merge_all()))
