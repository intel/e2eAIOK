...
def categorifyFeatures(df, proc, output_name, gen_dict, sampleRatio=1):
    ...
    op_feature_add_convert = FeatureAdd(
        cols={'most_used_word_bucket_id': "f.when(f.size(f.col('sorted_bucketized_tweet_word'))>0, f.col('sorted_bucketized_tweet_word').getItem(0)).otherwise('')",
             'second_used_word_bucket_id': "f.when(f.size(f.col('sorted_bucketized_tweet_word'))>1, f.col('sorted_bucketized_tweet_word').getItem(1)).otherwise('')"}, op='inline')
...
def get_encoding_features_dicts(proc):
    ...
    excludes = {'dt_dow': ['reply_timestamp', 'retweet_with_comment_timestamp', 'like_timestamp'],
               'tweet_type': ['reply_timestamp', 'retweet_timestamp'],
               'mentioned_bucket_id': ['reply_timestamp']
              }
