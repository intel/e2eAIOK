...
package com.intel.broaddp.test.dlrm
...
object JoinFillNAWithUDF {
  ...
  def categorify_udf(df: DataFrame, dict_df: DataFrame, col_name: String): DataFrame = {
    val sorted_data = dict_df.orderBy("model_count").collect()
    val broadcast_data: Array[(String, Int)] = sorted_data.zipWithIndex.map{case (row, idx) => (row(0).asInstanceOf[String], idx)}
    val broadcast_handler = spark.sparkContext.broadcast(broadcast_data.toMap)
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Save result to HDFS
    df.write.format("parquet").mode("overwrite").save(s"/dlrm/tmp/${randomUUID().toString}/")
