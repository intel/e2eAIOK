...
def evaluate(dataset, predictions):
    ...
    f1 = 100.0 * f1 / total
    return {'em': exact_match, 'qa_f1': f1}
...
def do_qa_eval(args, model, *inputs):
    all_results = []
    infer_times = []
    eval_dataloader, others = inputs
    examples = others[1]
    ...
    for batch_, example_indices in eval_dataloader:
        with torch.no_grad():
            ...
            batch_start_logits, batch_end_logits = total_logits.split(int(total_logits.size()[-1]/2), dim=-1)
