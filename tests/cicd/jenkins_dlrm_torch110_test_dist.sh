...
if [ $USE_SIGOPT == 1 ]; then
  printf "y\ny\n" | SIGOPT_API_TOKEN=$SIGOPT_API_TOKEN python run_hydroai.py --data_path $DATA_PATH --model_name $MODEL_NAME --conf tests/cicd/conf/hydroai_defaults_dlrm_dist_example.conf 2>&1 | tee /home/vmagent/app/cicd_logs/aidk_cicd_dlrm_torch110_$log_dir/aidk_cicd.log
else
  printf "y\ny\n" | python run_hydroai.py --data_path $DATA_PATH --model_name $MODEL_NAME --conf tests/cicd/conf/hydroai_defaults_dlrm_dist_example.conf --no_sigopt 2>&1 | tee /home/vmagent/app/cicd_logs/aidk_cicd_dlrm_torch110_$log_dir/aidk_cicd.log
fi
