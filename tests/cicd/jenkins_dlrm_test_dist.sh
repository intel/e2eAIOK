...
if [ $USE_SIGOPT == 1 ]; then
  ...
  python run_hydroai.py --no_sigopt --data_path $DATA_PATH --model_name $MODEL_NAME --conf $CONF_FILE --no_sigopt --custom_result_path $tmp_dir 2>&1 | tee $tmp_dir/aidk_cicd.log
fi
