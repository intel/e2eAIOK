...
CONF_FILE="/home/vmagent/app/e2eaiok/tests/cicd/conf/e2eaiok_defaults_dlrm_dist_example.conf"
...
if [ $USE_SIGOPT == 1 ]; then
  ...
  python run_e2eaiok.py --no_sigopt --data_path $DATA_PATH --model_name $MODEL_NAME --conf $CONF_FILE --no_sigopt --custom_result_path $tmp_dir 2>&1 | tee $tmp_dir/e2eaiok_cicd.log
fi
