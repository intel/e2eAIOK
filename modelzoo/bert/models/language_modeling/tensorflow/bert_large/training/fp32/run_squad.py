...
try:
  ...
  hvd.init()
  is_mpi = hvd.size()
  print("is_mpi:{}, rank:{}".format(is_mpi, hvd.rank()))
except ImportError:
  is_mpi = 0
...
flags.DEFINE_integer(
    ...
    "Number of hidden layers of Bert")

flags.DEFINE_float(
    "attention_probs_dropout_prob", None,
    "Attention layer probability of dropout")

flags.DEFINE_float(
    "hidden_dropout_prob", None,
    "Hidden layer probability of dropout")
...
def main(_):
  ...
  tf.compat.v1.logging.info("batch_size: {}, learning_rate: {}, num_hidden_layers: {}, attention_probs_dropout_prob: {}, hidden_dropout_prob: {}".format(FLAGS.train_batch_size, FLAGS.learning_rate, FLAGS.num_hidden_layers, FLAGS.attention_probs_dropout_prob, FLAGS.hidden_dropout_prob))

  if FLAGS.profile:
    tf.compat.v1.disable_eager_execution()

  bert_config = modeling.BertConfig.from_json_file(FLAGS.bert_config_file)
  if FLAGS.num_hidden_layers is not None:
  ...
  #  os.system("rm -rf {}".format(FLAGS.output_dir))
  #os.makedirs(FLAGS.output_dir)
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.profile ==True :
      ...
      hooks.append(tf.compat.v1.train.ProfilerHook(save_steps=30, output_dir=FLAGS.output_dir))
  ...
  if FLAGS.do_predict:
    ...
    tf.compat.v1.logging.info("Total Training Time:{}".format(total_training_time))
    ...
    write_predictions(eval_examples, eval_features, all_results,
                      ...
                      output_nbest_file, output_null_log_odds_file)
    with open(FLAGS.predict_file) as dataset_file:
      dataset_json = json.load(dataset_file)
      dataset = dataset_json["data"]
    with open(output_prediction_file) as test_file:
      predictions = json.load(test_file)
    predictions_results = evaluate(dataset, predictions)
    tf.compat.v1.logging.info("exact_match: {}, f1: {}".format(predictions_results["exact_match"], predictions_results["f1"]))
    with open(os.path.join(FLAGS.output_dir, "best_auc.txt"), "w+") as fout:
      fout.write(str(predictions_results["f1"]))

    end_time = time.time()
    total_time = end_time - start_time
    tf.compat.v1.logging.info("Total Time:{}".format(total_time))
    with open(os.path.join(FLAGS.output_dir,"best_time.txt"),"w+") as fout:
      fout.write(str(total_time)+"\n")
      fout.write(str(total_training_time))
