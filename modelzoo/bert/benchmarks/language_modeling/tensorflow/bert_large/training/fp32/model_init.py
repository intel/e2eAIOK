...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args=[], platform_util=None):
        ...
        arg_parser.add_argument('--test-file', help=' Do test', dest='test_file', default=None)
        arg_parser.add_argument('--num-to-evaluate', help= 'Frequency for testing', dest='num_to_evaluate', default=None)
        arg_parser.add_argument('--f1-threshold', help= 'F1 threshold for early stop', dest='f1_threshold', default=None)
        ...
        arg_parser.add_argument('--num_hidden_layers', help='Number of hidden layers', dest="num_hidden_layers", default=None)
        arg_parser.add_argument('--attention_probs_dropout_prob', help='Attention dropout probability', dest="attention_probs_dropout_prob", default=None)
        arg_parser.add_argument('--hidden_dropout_prob', help='Hidden layer dropout probability', dest="hidden_dropout_prob", default=None)
        arg_parser.add_argument('--host_file', help='hostfile for horovod run', default=None)
        ...
        if os.environ["MPI_NUM_PROCESSES"] == "None":
            ...
            if os.environ["MPI_NUM_PROCESSES_PER_SOCKET"] == "1":
                ...
                self.benchmark_command = "/opt/intel/oneapi/intelpython/latest/envs/tensorflow-2.5.0/bin/horovodrun --binding-args='-map-by socket' "
        ...
        if self.args.num_hidden_layers is not None:
            self.cmd_args = self.cmd_args + "--num_hidden_layers=" + str(self.args.num_hidden_layers) + eoo
        if self.args.attention_probs_dropout_prob is not None:
            self.cmd_args = self.cmd_args + "--attention_probs_dropout_prob=" + str(self.args.attention_probs_dropout_prob) + eoo
        if self.args.hidden_dropout_prob is not None:
            self.cmd_args = self.cmd_args + "--hidden_dropout_prob=" + str(self.args.hidden_dropout_prob) + eoo
        ...
        if self.args.train_option == "SQuAD":
            ...
            if self.args.test_file is not None:
                self.cmd_args = self.cmd_args + eoo + " --test_file=" + str(self.args.test_file)
            if self.args.num_to_evaluate is not None:
                self.cmd_args = self.cmd_args + eoo + " --num_to_evaluate=" + str(self.args.num_to_evaluate)
            if self.args.f1_threshold is not None:
                self.cmd_args = self.cmd_args + eoo + " --f1_threshold=" + str(self.args.f1_threshold)
        ...
        if os.environ["MPI_NUM_PROCESSES"] == "None":
            ...
            numa_cmd = "-np {} -H {} --network-interface {} "
            host_addr = ""
            host_interface =""
            with open(self.args.host_file, "r+") as fin:
                lines = fin.readlines()
                process_per_node = int(int(os.environ["MPI_NUM_PROCESSES"])/len(lines))
                for line in lines:
                    addr, interface = line.strip().split(":")
                    host_addr = host_addr + addr + ":" + str(process_per_node) + ","
                    host_interface = host_interface + interface + ","
                host_addr = host_addr[:-1]
                host_interface = host_interface[:-1]
            self.benchmark_command = self.benchmark_command + numa_cmd.format(int(os.environ["MPI_NUM_PROCESSES"]), host_addr, host_interface) + os.environ["PYTHON_EXE"] + " " \
                + benchmark_script + "\n"
