...
def prelu(_x, name=''):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        _alpha = tf.get_variable("prelu_" + name,
                                 _x.get_shape()[-1],
...
def attention(query,
    ...
    hidden_size = facts.get_shape().as_list()[
    ...
    w2 = tf.Variable(tf.random_normal([input_size, attention_size],
                                      stddev=0.1))
...
def self_attention(facts, ATTENTION_SIZE, mask, stag='null'):
    ...
    def body(batch, output, i):
        self_attention_tmp = din_fcn_attention(batch[:, i, :],
                                               batch[:, 0:i + 1, :],
                                               ATTENTION_SIZE,
                                               mask[:, 0:i + 1],
                                               softmax_stag=1,
