    ...
    def __init__(self, *,
        ...
        if synthetic_seq_len is None:
            if in_mem_file_list:
                ...
                file_list = "/tmp/rnnt_dali.file_list.5381423630832308739__0"
                self.read = ops.readers.File( name="Reader", dont_use_mmap=dont_use_mmap,
                                            pad_last_batch=(pipeline_type == 'val'), 
...
config_data = {}
config_data['max_duration'] = 16.7
config_data['sample_rate'] = 16000
config_data['speed_perturbation'] = {}
config_data['speed_perturbation']['min_rate'] = 0.85
config_data['speed_perturbation']['max_rate'] = 1.15
config_data['trim_silence'] = True

config_features = {}
config_features['window_size'] = 0.02
config_features['window_stride'] = 0.01
config_features['n_filt'] = 80
config_features['n_fft'] = 512
config_features['dither'] = 0.00001

dataset_path = '/mnt/sdd/LibriSpeech/LibriSpeech'

pipeline = DaliPipeline.from_config(config_data=config_data, config_features=config_features, device_id=None,
                                            file_root=dataset_path, sampler=None,
                                            device_type='cpu', batch_size=4,
                                            pipeline_type='train',
                                            num_cpu_threads=16,
                                            synthetic_seq_len=None, seed=2012,
                                            in_mem_file_list=False,
                                            pre_sort=False, dont_use_mmap=False)

pipeline.build()
out = pipeline.run()
audio, label, audio_len = out

audio_tensor = audio.as_tensor()
al_tensor = audio_len.as_tensor()
label_tensor = label.as_tensor()

print(np.array(audio.as_tensor()))
print(np.array(audio_len.as_tensor()))
print(np.array(label.as_tensor()))

print(audio_tensor.shape()) #3
print(al_tensor.shape()) #2
print(label_tensor.shape()) #2