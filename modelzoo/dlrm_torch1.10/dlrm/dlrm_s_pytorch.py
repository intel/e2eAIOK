...
class DLRM_Net(nn.Module):
    def create_mlp(self, ln, sigmoid_layer):
        ...
        for i in range(0, ln.size - 1):
            ...
            #     LL = ipex.nn.utils._weight_prepack._IPEXLinear(LL)
            ...
            if self.bf16 and self.use_ipex:
    ...
    def distributed_forward(self, dense_x, lS_o, lS_i):
        ...
        offset = torch.arange(batch_size * ext_dist.my_size)
    ...
    if not args.inference_only:
        ...
        if ext_dist.my_size == 1:
            ...
            if len(optimizers) == 1:
                ...
                # if args.bf16 and args.use_ipex:
                #     dlrm.emb_sparse, optimizer_sparse = ipex.optimize(dlrm.emb_sparse, dtype=torch.bfloat16, optimizer=optimizer_sparse)
                    # params_attr = {}
                    # ipex.optim._optimizer_utils.patch_load_state_dict(optimizer_sparse)
                    # setattr(optimizer_sparse, 'params_attr', params_attr)
                    # ipex.optim._optimizer_utils.patch_step_for_master_weight_training(optimizer_sparse)
                    # ipex.optim._optimizer_utils.patch_state_dict(optimizer_sparse)
                    # setattr(optimizer_sparse, 'fused', True)
                    # if not hasattr(optimizer_sparse, 'params_attr'):
                    #     setattr(optimizer_sparse, 'params_attr', {})
                    # if not hasattr(optimizer_sparse, '_original_step'):
                    #     setattr(optimizer_sparse, '_original_step', optimizer_sparse.step)
                    # setattr(optimizer_sparse, 'step', types.MethodType(ipex.optim._functional.sgd_step, optimizer_sparse))
                optimizer = (optimizer_dense, optimizer_sparse)
        ...
        while k < args.nepochs:
            ...
            for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if should_test and not args.inference_only:
                    ...
                    file1 = open("/home/vmagent/app/hydro.ai/modelzoo/dlrm_torch1.10/dlrm/best_auc.txt",'w')
