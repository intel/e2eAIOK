...
class SDA:
    def __init__(self, model, dataset_meta_path, train_path, eval_path, model_args):
        self.generate_config_file(model, dataset_meta_path, train_path, eval_path)
        self.load_config_file()
        if model == 'WnD':
            self.model_launcher = WnDLauncher(dataset_meta_path, train_path, eval_path, model_args)
        elif model == 'DLRM':
            self.model_launcher = DLRMLauncher(dataset_meta_path, train_path, eval_path, model_args)
        elif model == 'DIEN':
            self.model_launcher = DIENLauncher(dataset_meta_path, train_path, eval_path, model_args)
        else:
            raise RuntimeError(f'Model {model} is not supported!')
    
    def generate_config_file(self, model, dataset_meta_path, train_path, eval_path, file='sda.yaml'):
        config = {}
        config['train'] = {}
