...
do
	...
	echo "----------------------------------------------------------------"
	start=`date +%s%N`
	python script/train.py --mode=train --advanced --embedding_device=gpu --batch_size=$batch --num-intra-threads=20 --num-inter-threads=20 |& tee results/result_train_${batch}.txt
	end=`date +%s%N`
