...
def decode_cand_tuple(cand_tuple):
    print(F"cand_tiple:{cand_tuple}")
    depth = cand_tuple[0]
    return depth, list(cand_tuple[1:depth+1]), list(cand_tuple[depth + 1: 2 * depth + 1]), cand_tuple[-1]

class EvolutionSearcher(object):

    def __init__(self, args, device, model, choices, output_dir, model_type):
        self.device = device
        self.model = model
        self.args = args
    ...
    def is_legal(self, cand):
        ...
        nas_score = do_compute_nas_score(model_type = self.model_type, model=self.model, 
                                                        resolution=224,
                                                        batch_size=args.batch_size,
                                                        mixup_gamma=0.01)
        

        print(F"NAS Score:{nas_score}")
        info['acc'] = nas_score
    ...
    def search(self):
        ...
        print(F"Best mdoel structure:{self.keep_top_k[50][0]}")

        with open(os.path.join(args.output_dir, "best_model_structure.txt"), 'w') as f:
...
def get_args_parser():
    ...
    parser.add_argument('--param-limits', type=float, default=100)
...
def main(args):
    ...
    t = time.time()
    searcher = EvolutionSearcher(args, device, model, choices, args.output_dir, args.model_type)
