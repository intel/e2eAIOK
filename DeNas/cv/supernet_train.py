...
def decode_cand_tuple(cand_tuple):
    ...
    depth = int(cand_tuple[0])
    mlp_ratio = [float(x) for x in (cand_tuple[1:depth+1])]
    num_heads = [int(x) for x in (cand_tuple[depth + 1: 2 * depth + 1])]
    embed_dim = int(cand_tuple[-1])
...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='./data/imagenet/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='CIFAR10', choices=['CIFAR10', 'IMNET', 'INAT', 'INAT19'],
...
def main(args):
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device,  mode = args.mode)
